...
export default function PR() {
  ...
  return (
    <main className="min-h-screen w-screen flex flex-col items-center justify-center bg-gradient-to-br from-white via-gray-50 to-mint-400/10 px-4 md:px-8 py-16 md:py-28 font-sans" role="main">
      ...
      <section className="w-full max-w-3xl bg-gradient-glass rounded-2xl shadow-glass p-8 md:p-14 backdrop-blur-md border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {posts.map(post => (
            <article key={post.id} className="bg-white rounded-2xl shadow-soft p-6 hover:shadow-xl transition-all duration-200 hover:-translate-y-1 hover:scale-105 focus-within:shadow-xl focus-within:-translate-y-1 focus-within:scale-105 cursor-pointer border border-gray-100" tabIndex={0} aria-label={post.title}>
